...
export default function EmployeesPage() {
  ...
  // Form state (create / edit employee)
  const [formData, setFormData] = useState({
    id: "",
    firstName: "",
    lastName: "",
  ...
  const openModal = (emp?: any) => {
    if (emp) {
      setFormData({
        ...
        contact_information: emp.contact_information || "",
        status: emp.status,
      });
    } else {
      setFormData({
  ...
  const filtered = employees.filter((e) => {
    ...
    const matchesBranch = filterBranch === "ALL" || e.branch_id === filterBranch;
  ...
  return (
    <div className="page-content">
      ...
      <div className="section-header">
        <div>
          ...
          <p className="page-subtitle">
            Total {employees.length} employees registered
          </p>
        </div>
        <button className="btn btn-primary" onClick={() => openModal()}>
          <Plus size={16} /> Add Employee
      ...
      {/* Search bar + Filters (space‑between) */}
      <div className="card" style={{ marginBottom: "20px" }}>
        <div
